...
app.get("/", (req, res) => {
    res.send(`
        <h1>${APP_NAME}</h1>
        <p>Proyecto final - modulo 5</p>
        <p>Estado: aplicación base activa</p>
        <ul>
          <li><a href="/estado">Ver estado</a></li>
          <li><a href="/peticion.html">formulario de petición - proxima clase</a></li>
        </ul>
    `);
});
...
app.use((req, res) => {
    res.status(404).json({
      ...
      mensaje: "La ruta solicitada aun no existe o no ha sido creada."
    });
});
